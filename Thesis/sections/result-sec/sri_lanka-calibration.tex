...
\end{enumerate}
\footnotetext{
    Following \citet{zarazaga-12}, assume that the production function follows the form $y_t = h^\alpha_t k^{1-\alpha_t}$ where $y_t$ denotes output, $k_t$ denotes physical capital, and $h_t$ denotes employment. According to previous calibration, $ \alpha=$ 0.65, which implies that by the relationship ${y_t}/{h_t} = \left( {k_t}/{y_t} \right)^{0.35/0.65}$. If the capital-to-output ratio $\kappa_a ={k_a}/{y_a}$ falls to $\kappa_b = {k_{b}}/{y_b}$ after the default episode, and if we ascribe all the output loss to the loss of capital, we conclude that the output loss per worker is $\left({\kappa_a}/{\kappa_b} \right)^{0.35/0.65} -1$. According to capital stock data from the Penn World Table, for all default episodes, capital-output-ratio increases, indicating that the output would have actually increased based on this accounting. Therefore, I conclude that the approach by \citet{zarazaga-12} is not applicable in this scenario.}
