...
load simu_1950s.mat YT YTA YTtilde B D CT P W Q F TAU DEV PM INFL PC T Tburn ygrid pai filename rstar betta theta yhat alfa hbar a sigg xi dupper dlower nd 

%load historical data
load FinlandData.mat

FinlandSpread=FinlandSpread';
FinlandD=FinlandD';
FinlandNEER=FinlandNEER';
FinlandY=FinlandY';
netFinlandY=netFinlandY;
...
%start subplots
subplot(rows,cols,i)                                                                    
x=f(yT);plot(t,x,lineType,'linewidth',tslineWidth,'color',[0.4940 0.1840 0.5560])       
hold on 
...
h=legend('Model estimate of ${y^T_t}$','Historical detrended output, ${y_t}$, 1945-48','Historical detrended net output, ${y_t}-{d_t}$, 1945-48','Location','nw');
set(h, 'Interpreter', 'Latex')                                                      
xlim([-tb ta])                                                                       
...
set(h, 'Interpreter', 'Latex')
h=legend('Model estimate of Finnish credit spread','Historical spread vs. U.S. t-bills, 1945-48','Location','nw');
set(h, 'Interpreter', 'Latex')  
...
h=title('Debt level');
set(h, 'Interpreter', 'Latex')
h=legend('Model estimate of ${d_t}$','Historical debt-to-GDP, 1945-48','Location','nw');
set(h, 'Interpreter', 'Latex')  
...
h=title('Real wage');
set(h, 'Interpreter', 'Latex')
h=legend('Model estimate of ${w_t}$','Disposable real wage (1945 = 2), 1945-48','Location','sw');
set(h, 'Interpreter', 'Latex')  
...
set(h, 'Interpreter', 'Latex')
h=legend('Model estimate of ${\epsilon_t}$','Historical NEER (1945 = 1), 1945-48','Location','se');
set(h, 'Interpreter', 'Latex')  
...
set(h, 'Interpreter', 'Latex')
h=legend('Model estimate of ${p_t}$','Historical REER (1945 = 2.8), 1945-48','Location','sw');
set(h, 'Interpreter', 'Latex')  
